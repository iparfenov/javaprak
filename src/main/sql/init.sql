INSERT INTO website.employees (name, address, birthday, education, working_since, position, email, is_admin, login, password)
VALUES
('Светлана Ищенко', 'г. Москва, улица Улица дом 5 кв. 25', '2000-01-15', 'высшее', '2024-09-10', 'директор', 'director@company.org', True, 'svetlana', 'passwordThatDoesNotPissOffGoogle'),
('Екатерина Андреева', 'г. Москва, ...', '1999-04-04', 'высшее', '2025-01-15', 'менеджер', 'manager@company.org', True, 'ekaterina', 'passwordThatDoesNotPissOffGoogle'),
('Александр Погодин', 'г. Москва ...', '1995-12-15', 'высшее', '2023-09-10', 'программист', 'employee@company.org', False, 'alexander', 'passwordThatDoesNotPissOffGoogle');

INSERT INTO website.projects (project_name, project_start, project_end, project_head)
VALUES
('проект1', '2025-02-15', '2025-05-01', 1),
('проект2', '2025-01-01', '2025-01-15', 1),
('проект3', '2025-01-17', '2025-02-27', 3);

INSERT INTO website.employees_projects (employee_id, project_id, position, appointed_at, quit_at)
VALUES
(1, 1, 'директор', '2025-02-15', NULL),
(2, 1, 'менеджер', '2025-02-15', NULL),
(1, 2, 'директор', '2025-01-01', '2025-01-15'),
(2, 2, 'менеджер', '2025-02-15', '2025-01-15'),
(2, 3, 'менеджер', '2025-02-15', NULL),
(3, 3, 'программист', '2025-02-15', NULL);

INSERT INTO website.bonuses (name, percentage)
VALUES
('Новогодняя премия', 200.00),
('Премия на день рождения', 50.00),
('Премия за усердную работу', 100.00);

INSERT INTO website.employees_payouts (employee_id, amount, paid_at, bonus_id)
VALUES
(1, 300000.00, '2025-01-25', NULL),
(1, 450000.00, '2025-02-10', 2),
(2, 150000.00, '2025-01-25', 2),
(2, 100000.00, '2025-02-10', NULL),
(3, 260000.00, '2025-01-25', 3),
(3, 130000.00, '2025-02-10', NULL);

INSERT INTO website.employee_history (employee_id, positions, promoted_at)
VALUES
(1, ARRAY ['директор'], ARRAY [DATE '2024-09-10']),
(2, ARRAY ['менеджер'], ARRAY [DATE '2025-01-15']),
(3, ARRAY ['стажер', 'программист'], ARRAY [DATE '2023-09-10', DATE '2024-02-15']);

COMMIT;
